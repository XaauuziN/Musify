...
class music(commands.Cog):
    ...
    async def play(self, interaction:discord.Interaction,busca:str):
        ...
        try:
            ...
            embedvc = discord.Embed(
                colour= 1646116,#grey
                description = 'Entra num canal de voz primeiro seu animal.'
            )
            ...
            if type(song) == type(True):
                embedvc = discord.Embed(
                    colour= 12255232,#red
                    description = 'Algo de errado não está certo! Tente mudar ou configurar a playlist/vídeo ou escrever o nome dele novamente!'
                )
                await interaction.followup.send(embed=embedvc)
            else:
                embedvc = discord.Embed(
                    colour= 32768,#green
                    description = f"Lançou a braba! **{song['title']}** à fila!"
                )
    ...
    async def q(self, interaction:discord.Interaction):
        ...
        if retval != "":
            ...
            embedvc = discord.Embed(
                colour= 1646116,
                description = 'Bota música ai bixu, tem nada pra escutar.'
            )
            await interaction.followup.send(embed=embedvc)
    ...
    async def pular(self, interaction:discord.Interaction):
        ...
        if self.vc != "" and self.vc:
            ...
            embedvc = discord.Embed(
                ...
                description = f"Da puta ocê, puque tira mùsica meu."
            )
            await interaction.followup.send(embed=embedvc)
    ...
    async def skip_error(self,interaction: discord.Interaction, error: app_commands.AppCommandError):
        if isinstance(error, commands.MissingPermissions):
            embedvc = discord.Embed(
                colour= 12255232,
                description = f"Não deixei oce pular minha musica eu, eu num deixei."
            )
            await interaction.followup.send(embed=embedvc)     
        else:
            raise error
